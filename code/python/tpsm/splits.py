...
def make_repeated_kfold_splits(
    ...
    k_used = k
    if target_col and target_col in df.columns and task_name == "classification":
        y = df[target_col]

        # Stratify for all classification tasks (categorical or encoded numeric)
        try:
            ...
            rkf = RepeatedStratifiedKFold(
                n_splits=k, n_repeats=repeats, random_state=base_seed
            )
            if effective_k != k:
                rkf = RepeatedStratifiedKFold(
...
def make_splits(
    ...
    if method == "rolling_origin" or task_name == "timeseries":
        ...
        return make_repeated_kfold_splits(
            df, split_cfg, target_col, dataset_id, task_name=task_name
        )
