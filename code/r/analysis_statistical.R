...
source("code/r/tpsm/analysis_statistical.R")

args <- commandArgs(trailingOnly = TRUE)

INPUT_CSV <- if (length(args) >= 1) args[1] else "outputs/active/r/combined_pairwise_differences.csv"
OUTPUT_DIR <- if (length(args) >= 2) args[2] else "outputs/active/r/analysis"
