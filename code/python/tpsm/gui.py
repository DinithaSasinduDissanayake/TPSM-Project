...
from code.python.tpsm.run_state import load_json, utc_now
...
def parse_args():
    ...
    parser.add_argument("--output-root", default="outputs/active/gui_runs")
...
def _dataset_catalog() -> dict[str, list[dict]]:
    base = _load_raw_yaml(os.path.join(project_root, "config", "production", "datasets.yaml"))
    out = {}
    for task_name in ("classification", "regression", "timeseries"):
...
def _build_config_from_form(form: dict[str, list[str]]) -> tuple[str, dict]:
    ...
    if preset == "quick":
        base_path = os.path.join(project_root, "config", "smoke", "smoke_test.yaml")
    else:
        base_path = os.path.join(project_root, "config", "production", "datasets.yaml")
...
def _render_active_run_buttons(run_id: str, status: str) -> str:
    rid = html.escape(run_id)
    buttons = [
        f'<a class="btn btn-ghost" href="/runs/{rid}">&#128269; Open Run</a>',
    ]
    if status == "running":
        buttons.append(
            f'<form method="post" action="/runs/{rid}/pause" style="display:inline">'
            f'<button class="btn btn-ghost" type="submit">&#9646;&#9646; Pause</button>'
            f"</form>"
        )
        buttons.append(
            f'<form method="post" action="/runs/{rid}/stop" style="display:inline">'
            f'<button class="btn btn-danger" type="submit">&#9632; Stop</button>'
            f"</form>"
        )
    elif status == "paused":
        buttons.append(
            f'<form method="post" action="/runs/{rid}/resume" style="display:inline">'
            f'<button class="btn btn-success" type="submit">&#9654; Resume</button>'
            f"</form>"
        )
        buttons.append(
            f'<form method="post" action="/runs/{rid}/stop" style="display:inline">'
...
def _spawn_runner(
    ...
    if resume:
        cmd = [python_bin, "-m", "code.python.tpsm.main", "--resume-run", run_dir]
    else:
        cmd = [
            python_bin,
            "-m",
...
<div class="hero-card full-width">
  ...
  <div class="btn-row">{_render_active_run_buttons(active_run["run_id"], status)}</div>
</div>
...
<div class="page">
  ...
  <div class="btn-row" style="margin-bottom:24px">
    {_render_active_run_buttons(run_id, run_status).replace('>Pause<', '>Pause After Split<').replace('>Stop<', '>Stop After Split<')}
  </div>
...
def main():
    ...
    archive_old_complete_runs(args.output_root, keep_run_id="")
    Handler.args = args
