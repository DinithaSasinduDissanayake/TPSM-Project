...
def build_execution_plan(cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for task_name in task_order:
        ...
        for ds_cfg in task["datasets"]:
            ...
            splits = make_splits(task_name, df, split_cfg, ds_cfg["target"], ds_cfg["id"])
...
def _progress_from_state(state: dict[str, Any]) -> dict[str, Any]:
    ...
    completed_datasets = sum(1 for d in datasets.values() if d["completed_units"] >= d["total_units"])
...
class RunStateStore:
    ...
    def mark_unit_completed(self, state: dict[str, Any], unit_id: str) -> None:
        for unit in state["units"]:
            if unit["unit_id"] == unit_id:
                ...
                dataset["status"] = (
                    "completed" if dataset["completed_units"] >= dataset["total_units"] else "running"
                )
    ...
    def mark_dataset_failed(self, state: dict[str, Any], task_name: str, dataset_id: str) -> None:
    ...
    def completed_unit_ids_for_dataset(self, state: dict[str, Any], task_name: str, dataset_id: str) -> set[str]:
        dkey = make_dataset_key(task_name, dataset_id)
        return {
            unit["unit_id"]
            for unit in state["units"]
    ...
    def aggregate_outputs(self) -> tuple[pd.DataFrame, pd.DataFrame, list[Any]]:
        ...
        for name in sorted(os.listdir(self.model_parts_dir)):
            if name.endswith(".csv"):
                model_frames.append(pd.read_csv(os.path.join(self.model_parts_dir, name)))
        for name in sorted(os.listdir(self.pairwise_parts_dir)):
            if name.endswith(".csv"):
                pair_frames.append(pd.read_csv(os.path.join(self.pairwise_parts_dir, name)))
        ...
        if not model_df.empty and "run_id" in model_df.columns:
            model_df = model_df.drop_duplicates(subset=["run_id"]).reset_index(drop=True)
        if not pair_df.empty and "comparison_id" in pair_df.columns:
            pair_df = pair_df.drop_duplicates(subset=["comparison_id"]).reset_index(drop=True)
