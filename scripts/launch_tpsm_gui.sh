...
PYTHON_BIN="${PROJECT_ROOT}/scripts/python/.venv/bin/python"
HOST="127.0.0.1"
PORT="8787"
URL="http://${HOST}:${PORT}"
OUTPUT_ROOT="${PROJECT_ROOT}/outputs/gui_runs"
...
kill_matching_gui_processes() {
  local pids
  pids="$(pgrep -f "${PYTHON_BIN} -m scripts.python.gui --host ${HOST} --port ${PORT}" || true)"
  if [[ -n "${pids}" ]]; then
...
start_gui() {
  (
    cd "${PROJECT_ROOT}"
    nohup "${PYTHON_BIN}" -m scripts.python.gui --host "${HOST}" --port "${PORT}" --output-root "${OUTPUT_ROOT}" \
      >>"${LOG_FILE}" 2>&1 &
    echo $! > "${PID_FILE}"
  )
}
