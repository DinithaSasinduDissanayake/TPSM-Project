make_lag_matrix <- function(y, max_lag = 12, exog = NULL, exog_max_lag = 6) {
  ...
  for (l in seq_len(max_lag)) {
    out[[paste0("lag_", l)]] <- c(rep(NA_real_, l), y[seq_len(n - l)])
  }
  if (!is.null(exog) && is.data.frame(exog)) {
    for (col in names(exog)) {
      exog_n <- length(exog[[col]])
      for (l in seq_len(exog_max_lag)) {
        out[[paste0(col, "_lag_", l)]] <- c(rep(NA_real_, l), exog[[col]][seq_len(exog_n - l)])
      }
    }
  }
  stats::na.omit(out)
...
train_predict_timeseries <- function(model_name, y_train, y_test, lag = 12, exog_train = NULL, exog_test = NULL) {
  if (model_name == "arima") {
    ...
    if (has_exog && forecast_available) {
      # ARIMAX with auto.arima for fast stepwise order selection
      fit <- tryCatch({
        forecast::auto.arima(y_train, xreg = exog_train, stepwise = TRUE, approximation = TRUE)
      }, error = function(e) {
        warning(sprintf(
          "auto.arima with xreg failed: %s. Falling back to ARIMA(1,1,1).", e$message
        ))
        forecast::Arima(y_train, order = c(1, 1, 1), xreg = exog_train)
      })
      fc <- forecast::forecast(fit, h = length(y_test), xreg = exog_test)$mean
      return(as.numeric(fc))
    } else if (forecast_available) {
      # Standard ARIMA with auto.arima (no exogenous variables)
      fit <- tryCatch({
        forecast::auto.arima(y_train, stepwise = TRUE, approximation = TRUE)
      }, error = function(e) {
        warning(sprintf(
          "auto.arima failed: %s. Falling back to ARIMA(1,1,1).", e$message
        ))
        forecast::Arima(y_train, order = c(1, 1, 1))
      })
      fc <- forecast::forecast(fit, h = length(y_test))$mean
      return(as.numeric(fc))
    } else {
      # Fallback: manual grid search when forecast package is not available
      best_aic <- Inf
      best_order <- c(1, 1, 1)
      for (p in 0:2) {
        for (d in 0:1) {
  ...
  if (model_name == "gbm_lag") {
    ...
    for (i in seq_along(y_test)) {
      # Extract only the last `lag` values for features (O(h) instead of O(n*h))
      # This is mathematically identical to taking the last row of make_lag_matrix()
      recent_vals <- rev(tail(current_y, lag + 1))[-1]  # Drop current value, reverse so lag_1=most recent
      x_step <- recent_vals

      # Add exog lags if present
      if (!is.null(current_exog_test)) {
        all_exog <- rbind(current_exog_train, current_exog_test[1:i, , drop = FALSE])
        for (col in names(all_exog)) {
          recent_exog <- rev(tail(all_exog[[col]], exog_max_lag + 1))[-1]
          x_step <- c(x_step, recent_exog)
        }
      }

      test_lag_df <- data.frame(matrix(x_step, nrow = 1))
      names(test_lag_df) <- paste0("x", seq_along(x_step))
