...
def profile_dataset_for_cleaning(df, target):
    """Profile dataset for lab-style cleaning summaries."""
    ...
    return {
        ...
        "constant_feature_cols": int(
            sum(feature_df[col].nunique(dropna=False) <= 1 for col in feature_df.columns)
        )
        if not feature_df.empty
        else 0,
        "numeric_feature_cols": int(len(numeric_cols)),
        "numeric_outlier_cells": int(outlier_cells),
        "target_na_rows": int(df[target].isna().sum()) if target in df.columns else 0,
    }


def build_dataset_cleaning_summary(task_name, ds_cfg, raw_df, prepared_df):
    """Build one dataset-level summary row for reportable cleaning."""
    raw_prof = profile_dataset_for_cleaning(raw_df, ds_cfg["target"])
    prepared_prof = profile_dataset_for_cleaning(prepared_df, ds_cfg["target"])
    return {
        "task_type": task_name,
        "dataset_id": ds_cfg["id"],
        "dataset_source": ds_cfg.get("source", "unknown"),
        "target_column": ds_cfg["target"],
        "raw_rows": raw_prof["rows"],
        "raw_cols": raw_prof["cols"],
        "prepared_rows": prepared_prof["rows"],
        "prepared_cols": prepared_prof["cols"],
        "rows_removed_total": int(raw_prof["rows"] - prepared_prof["rows"]),
        "cols_removed_total": int(raw_prof["cols"] - prepared_prof["cols"]),
        "target_na_rows_before": raw_prof["target_na_rows"],
        "missing_cells_before": raw_prof["missing_cells"],
        "missing_cells_after_prepare": prepared_prof["missing_cells"],
        "duplicate_rows_before": raw_prof["duplicate_rows"],
        "duplicate_rows_after_prepare": prepared_prof["duplicate_rows"],
        "constant_feature_cols_before": raw_prof["constant_feature_cols"],
        "constant_feature_cols_after_prepare": prepared_prof["constant_feature_cols"],
        "numeric_feature_cols_before": raw_prof["numeric_feature_cols"],
        "numeric_outlier_cells_before": raw_prof["numeric_outlier_cells"],
        "numeric_outlier_cells_after_prepare": prepared_prof["numeric_outlier_cells"],
        "force_binary": bool(ds_cfg.get("force_binary", False)),
        "notes": None,
    }
...
def preprocess_for_modeling(df, target, task_name, ds_cfg):
    ...
    # Drop rows where target is missing - do this BEFORE any target conversion
    ...
    if task_name == "classification" and ds_cfg.get("force_binary"):
        ...
        if threshold is not None:
            df[target] = (
                pd.to_numeric(df[target], errors="coerce") > threshold
            ).astype(int)
        elif pos_vals:
            df[target] = (
                df[target]
                .astype(str)
                .str.strip()
                .isin([str(v).strip() for v in pos_vals])
                .astype(int)
            )
    ...
    if task_name == "classification" and _is_string_col(df[target]):
        ...
        # Encode positive_class to match the label-encoded target
        pos_class = ds_cfg.get("positive_class")
        if pos_class is not None:
            pos_str = str(pos_class)
            if pos_str in le.classes_:
                ds_cfg["_encoded_positive_class"] = int(le.transform([pos_str])[0])
...
def preprocess_train_test_for_modeling(train_df, test_df, target, task_name, ds_cfg):
    ...
    # Drop rows with missing target - do this BEFORE any target conversion
    ...
    if (
        ...
        and ds_cfg.get("force_binary")
        and not _target_already_binary(train_df[target])
    ):
        threshold = ds_cfg.get("binary_threshold")
        pos_vals = ds_cfg.get("binary_positive_vals")
        if threshold is not None:
            train_df[target] = (
                pd.to_numeric(train_df[target], errors="coerce") > threshold
            ).astype(int)
            test_df[target] = (
                pd.to_numeric(test_df[target], errors="coerce") > threshold
            ).astype(int)
        elif pos_vals:
            norm_pos = [str(v).strip() for v in pos_vals]
            train_df[target] = (
                train_df[target].astype(str).str.strip().isin(norm_pos).astype(int)
            )
            test_df[target] = (
                test_df[target].astype(str).str.strip().isin(norm_pos).astype(int)
            )
    ...
    candidate_cols = [
        c for c in train_df.columns if c != target and c in test_df.columns
    ]
    for col in candidate_cols:
        if _is_string_col(train_df[col]) or _is_string_col(test_df[col]):
            ...
            sample_vals = pd.concat(
                [tr_vals.head(20), te_vals.head(20)], ignore_index=True
            )
            has_date_hint = bool(
            ...
            if has_date_hint:
                ...
                if tr_dt_ok >= 0.9 and te_dt_ok >= 0.9:
                    train_df[col] = tr_dt.apply(
                        lambda x: x.timestamp() if pd.notna(x) else np.nan
                    )
                    test_df[col] = te_dt.apply(
                        lambda x: x.timestamp() if pd.notna(x) else np.nan
                    )
                    continue
...
def build_pairwise_rows(
    ...
    metrics_to_compare = (
        requested_metrics if requested_metrics else list(single_metrics.keys())
    )
    ...
    for metric_name in metrics_to_compare:
        ...
        if higher_better:
            ...
            )  # positive = ensemble better (lower is better)
        ...
        rows.append(
            {
                ...
                "abs_difference_value": round(abs(diff), 6),
                "model_pair": f"{single_name}__vs__{ensemble_name}",
            }
        )
...
def evaluate_models_on_split(
    ...
    for model_name in model_names:
        ...
        # Determine effective metrics: use configured list, or fallback later
        configured_metrics = ds_cfg.get("metrics")
        ...
        try:
            with warnings.catch_warnings(record=True) as caught:
                ...
                t_train = time.time()
                result = run_model(
                    task_name, model_name, train_df, test_df, target, ds_cfg
                )
                ...
                metrics = calc_metrics(
                    ...
                    result["y_pred"],
                    result.get("y_prob"),
                    positive_class=ds_cfg.get(
                        "_encoded_positive_class", ds_cfg.get("positive_class")
                    ),
                )
            ...
            # Determine effective metrics: use configured list, or all computed metrics
            effective_metrics = (
                configured_metrics if configured_metrics else list(metrics.keys())
            )

            for metric_name in effective_metrics:
                val = metrics.get(metric_name, np.nan)
                model_run_id = f"run::{run_id}::{task_name}::{ds_cfg['id']}::{model_name}::{split_info['fold']}::{split_info['repeat_id']}::{metric_name}"
                model_rows.append(
                    {
                        "run_id": run_id,
            ...
            # Use configured metrics if available, else empty (model failed so no computed metrics)
            effective_metrics = configured_metrics if configured_metrics else []
            for metric_name in effective_metrics:
                model_run_id = f"run::{run_id}::{task_name}::{ds_cfg['id']}::{model_name}::{split_info['fold']}::{split_info['repeat_id']}::{metric_name}"
                model_rows.append(
                    {
...
def run_dataset_task(
    ...
    effective_timeout_sec = ds_cfg.get("timeout_seconds") or timeout_sec

    def _check_timeout(stage: str):
        if effective_timeout_sec is None or effective_timeout_sec <= 0:
            return
        elapsed = time.time() - start_time
        if elapsed > effective_timeout_sec:
            raise TimeoutError(
                f"Dataset '{ds_cfg['id']}' for task '{task['name']}' exceeded timeout "
                f"of {effective_timeout_sec}s during stage '{stage}' (elapsed {elapsed:.2f}s)."
            )
    ...
    try:
        ...
        log_fn(
            "info",
            "dataset_loaded",
            {"dataset": ds_cfg["id"], "rows": len(df), "cols": len(df.columns)},
        )
        if ds_cfg.get("_column_match_diagnostics", {}).get("renamed_columns"):
            log_fn(
                "info",
                "dataset_column_match",
                {
                    "dataset": ds_cfg["id"],
                    "renamed_columns": ds_cfg["_column_match_diagnostics"][
                        "renamed_columns"
                    ],
                },
            )
        ...
        result["dataset_summary"] = build_dataset_cleaning_summary(
            task["name"], ds_cfg, raw_df, df
        )
        prep_sec = time.time() - t1
        _check_timeout("prepare")
        log_fn(
            "debug",
            "dataset_stage_complete",
            {
        ...
        splits = make_splits(
            task["name"], df, split_cfg, ds_cfg["target"], ds_cfg["id"]
        )
        if not splits:
            raise ValueError(
                f"Dataset '{ds_cfg['id']}' for task '{task['name']}' generated 0 splits."
            )
        split_sec = time.time() - t2
        _check_timeout("split")
        log_fn(
            "debug",
            "dataset_stage_complete",
            {
        ...
        for split_idx, split_info in enumerate(splits, start=1):
            _check_timeout(f"split_{split_idx}")
            ...
            if task["name"] in ("regression", "timeseries"):
                ...
                if valid.any():
                    ...
                    if low_pct > 10.0:
                        ...
                        metric_notes["smape"] = (
                            f"low_target_context:{low_pct:.1f}%_<=10"
                        )
            ...
            for pair in task.get("model_pairs", []):
                ...
                for row in pair_rows:
                    row["dataset_source"] = ds_cfg.get("source", "unknown")
                    row["train_rows"] = len(train_df)
                    row["test_rows"] = len(test_df)
                    row["dataset_total_rows"] = (
                        result["dataset_summary"]["prepared_rows"]
                        if result["dataset_summary"]
                        else len(df)
                    )
                    row["dataset_total_cols"] = (
                        result["dataset_summary"]["prepared_cols"]
                        if result["dataset_summary"]
                        else len(df.columns)
                    )
            ...
            _check_timeout(f"split_{split_idx}_complete")
