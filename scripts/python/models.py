...
def train_svr(X_train, y_train, **kwargs):
    ...
    if len(y_train) > MAX_SVR_ROWS:
        idx = np.random.RandomState(42).choice(len(y_train), MAX_SVR_ROWS, replace=False)
        X_train = X_train[idx]
        y_train = y_train[idx]
    ...
    with warnings.catch_warnings():
        ...
        try:
            model = pm.auto_arima(
                ...
                error_action='ignore', max_order=max_order,
                seasonal=False,
            )
            return model
        except Exception:
...
def predict_arima(model, n_steps, exog_test=None):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        # pmdarima model uses predict(), statsmodels uses forecast()
...
def train_gbm_lag(y_train, exog_train=None, n_lags=12, exog_max_lag=6, **kwargs):
    ...
    for i in range(start, n):
        ...
        if exog_train is not None:
            ex_row = exog_train[i] if hasattr(exog_train[i], '__len__') else [exog_train[i]]
            n_exog = len(ex_row)
            for c in range(n_exog):
                for lag in range(1, exog_max_lag + 1):
                    row.append(exog_train[i - lag][c] if hasattr(exog_train[i - lag], '__getitem__') else exog_train[i - lag])
        data.append(row)
...
def predict_gbm_lag(model_tuple, y_history, n_steps, exog_test=None):
    ...
    for i in range(n_steps):
        ...
        if exog_test is not None and exog_max_lag > 0 and i < len(exog_test):
            ...
            ex = exog_test[i]
            ex_list = ex.tolist() if hasattr(ex, "tolist") else ([ex] if not hasattr(ex, '__len__') else list(ex))
...
def run_model(task_name, model_name, train_df, test_df, target, ds_cfg):
    ...
    if task_name == "timeseries":
        ...
        return _run_tabular_model(task_name, resolved_name, train_df, test_df, target, ds_cfg)
...
def _run_timeseries_model(model_name, train_df, test_df, target, ds_cfg):
    ...
    if exog_cols:
        ...
        if ecols:
            ...
            if model_name in ("arima", "exp_smoothing") and len(exog_train) > MAX_TS_ROWS:
